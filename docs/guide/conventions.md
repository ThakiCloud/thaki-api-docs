...
`pageSize` 기본값은 서비스·리소스마다 다릅니다(컴퓨트 10, 네트워크 20, 컨테이너는 워크로드 20·그 외 10). `page=0`은 전체 조회로 쓰이지만, 컨테이너 Namespace API는 이를 지원하지 않습니다.
...
컴퓨트 API는 헤더 검증이 인증보다 먼저 실행됩니다. 조직·파티션 헤더가 없으면 토큰 유무와 관계없이 422를 반환하며, 헤더가 갖춰진 뒤에야 토큰 누락·만료가 401로 판정됩니다. 서비스별 세부 규칙은 각 [API 레퍼런스](/api/) 페이지를 참조하십시오.
:::
