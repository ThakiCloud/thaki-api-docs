...
Action 목록을 조회합니다. 필터링 및 페이지네이션을 지원합니다. app, resourceType 조건 중 하나만 있으면 해당 조건 일치, 둘 다 있으면 AND 조건 적용.

## HTTP 요청

```http
GET https://<your-console-host>/api/v1/iam/authz/action-catalog/actions
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| app | 선택 | string 또는 null | App 필터 (대소문자 무관) |
| alias | 선택 | string 또는 null | Alias 필터 |
| method | 선택 | string 또는 null | HTTP Method 필터 |
| resourceType | 선택 | string 또는 null | 리소스 타입 필터 (대소문자 무관, 예: instance, network) |
| actionClass | 선택 | string 또는 null | Action Class 필터 (대소문자 무관, 예: Read, List, Write, Delete, Admin) |
| page | 선택 | integer | 페이지 번호 (0=전체 조회). 기본값 1. 범위 0~ |
| pageSize | 선택 | integer | 페이지 크기. 기본값 20. 범위 1~1000 |
