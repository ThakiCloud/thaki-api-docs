# 관리자 전용: 모든 PDP 평가 및 시뮬레이션 감사 로그 조회
...
| pageSize | 선택 | integer | 페이지 크기 (최대 500). 기본값 20. 범위 1~500 |
| sort | 선택 | string 또는 null | 정렬 필드 |
| order | 선택 | string 또는 null | 정렬 방향 (asc \| desc) |
| timeGte | 선택 | string (date-time) | 조회 시작 시간 이상 (ISO8601) |
| timeLte | 선택 | string (date-time) | 조회 종료 시간 이하 (ISO8601) |
| eventType | 선택 | array (string) | 이벤트 타입 (pdp_evaluation, policy_simulation 등, 다중값 OR) |
| auditId | 선택 | string 또는 null | 감사 로그 ID (부분 매칭) |
| principalTpn | 선택 | string 또는 null | Principal TPN (부분 매칭) |
| resourceTrn | 선택 | string 또는 null | Resource TRN (부분 매칭) |
| ipAddress | 선택 | string 또는 null | IP 주소 (부분 매칭) |
| action | 선택 | string 또는 null | Action (부분 매칭) |
| decision | 선택 | string 또는 null | 결정 결과 (Allow/Deny) |
