# API Key의 이름(설명) 및/또는 활성 상태 수정

API Key의 이름(설명) 및/또는 활성 상태를 수정합니다.

## HTTP 요청

```http
PATCH https://<your-console-host>/api/v1/iam/authn/api-keys/{key_id}
```
...
| 이름 | 위치 | 필수 | 형식 | 설명 |
|---|---|---|---|---|
| key_id | path | 필수 | string | API Key ID |
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| name | 선택 | string 또는 null | Key 이름(설명). 길이 0~128 |
| enabled | 선택 | boolean 또는 null | 키 활성/비활성 (true=activate, false=deactivate, 생략=변경 없음) |
