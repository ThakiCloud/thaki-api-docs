# 정책에 바인딩된 역할 목록을 조회합니다 (활성 부여 수 포함)
...
| name | 선택 | string 또는 null | Role 이름 부분 일치 필터 |
| id | 선택 | string 또는 null | Role ID 부분 일치 필터 |
| description | 선택 | string 또는 null | 설명 부분 일치 필터 |
| activeGrantsGte | 선택 | integer 또는 null | 활성 부여 수 최소값 (포함) |
| activeGrantsLte | 선택 | integer 또는 null | 활성 부여 수 최대값 (포함) |
