...
조직의 비밀번호 정책을 업데이트합니다.

## HTTP 요청

```http
PUT https://<your-console-host>/api/v1/iam/authn/organizations/{org_id}/password-policy
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| minLength | 선택 | integer 또는 null | 최소 길이. 범위 6~128 |
| maxLength | 선택 | integer 또는 null | 최대 길이. 범위 6~128 |
| requireUppercase | 선택 | boolean 또는 null | 대문자 필수 여부 |
| requireLowercase | 선택 | boolean 또는 null | 소문자 필수 여부 |
| requireDigit | 선택 | boolean 또는 null | 숫자 필수 여부 |
| requireSpecial | 선택 | boolean 또는 null | 특수문자 필수 여부 |
| passwordHistoryCount | 선택 | integer 또는 null | 비밀번호 재사용 방지 개수. 범위 0~24 |
| expiryDays | 선택 | integer 또는 null | 비밀번호 만료 기간 (일). 범위 0~1095 |
| minAgeDays | 선택 | integer 또는 null | 비밀번호 최소 사용 기간 (일). 범위 0~1095 |
| excludeUsername | 선택 | boolean 또는 null | 비밀번호에 사용자명 포함 금지 |
| excludeEmail | 선택 | boolean 또는 null | 비밀번호에 이메일 포함 금지 |
| passwordHistoryPeriodDays | 선택 | integer 또는 null | 최근 N일간 비밀번호 재사용 방지. 범위 0~365 |
