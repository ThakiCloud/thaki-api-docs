...
```http
POST https://<your-console-host>/api/v1/compute/storage/volume-snapshots
```
...
| volumeSnapshotName | 필수 | string | 볼륨 스냅샷 이름 (1~255자, 인쇄 가능 문자, 선행/후행 공백 금지) |
| description | 선택 | string 또는 null | 볼륨 스냅샷 설명 (선택) |
| force | 선택 | boolean | 강제 생성 여부 (볼륨이 available 상태가 아니어도 생성 시도). 기본값 false |
