...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| sort | 선택 | string | 정렬 필드. 정렬 필드. 값: flavorName, flavorId, cpuCount, ram, disk |
| order | 선택 | string | 정렬 방향. 정렬 방향. 값: asc, desc |
| flavorType | 필수 | string | Flavor 타입 (cpu/gpu/npu/bareMetal). Flavor 타입 (cpu/gpu/npu/bareMetal). 값: cpu, gpu, npu, bareMetal |
| flavorId | 선택 | array (string) | Flavor ID 필터 (부분 일치, 다중 값 OR). Flavor ID 필터 (부분 일치, 다중 값 OR) |
| flavorName | 선택 | array (string) | Flavor 이름 필터 (부분 일치, 다중 값 OR). Flavor 이름 필터 (부분 일치, 다중 값 OR) |
| extraSpec | 선택 | array (string) | extra_specs 검색 필터 (key/value 구분 없음, 부분 일치, 다중 값 OR). extra_specs 검색 필터 (key/value 구분 없음, 부분 일치, 다중 값 OR) |
| cpuCount | 선택 | array (integer) | CPU 수 필터 (완전 일치, 다중 값 OR). CPU 수 필터 (완전 일치, 다중 값 OR) |
| cpuCountGte | 선택 | integer 또는 null | CPU 수 이상 필터 (vCPU &gt;= N). Container 서비스에서 호출. CPU 수 이상 필터 (vCPU &gt;= N). Container 서비스에서 호출. 범위 0~ |
| ram | 선택 | array (number) | RAM 크기 필터 GiB 단위 (완전 일치, 다중 값 OR). RAM 크기 필터 GiB 단위 (완전 일치, 다중 값 OR) |
| disk | 선택 | array (integer) | Disk 필터 GiB 단위 (완전 일치, 다중 값 OR). Disk 필터 GiB 단위 (완전 일치, 다중 값 OR) |
| isPublic | 선택 | boolean 또는 null | 공개 여부 필터 (true/false). 공개 여부 필터 (true/false) |
| pool | 선택 | array (string) | AZ pool 필터 (shared/dedicated/gpu 등, 완전 일치, 다중 값 OR). instance 생성 시 선택 AZ에 맞는 flavor만 노출하기 위해 사용.. AZ pool 필터 (shared/dedicated/gpu 등, 완전 일치, 다중 값 OR). instance 생성 시 선택 AZ에 맞는 flavor만 노출하기 위해 사용. |
