...
```http
POST https://<your-console-host>/api/v1/compute/instances
```
...
| result.networks | 선택 | array (object) | 네트워크 목록 |
| result.networks[].networkId | 선택 | string 또는 null | 네트워크 ID |
| result.networks[].networkName | 선택 | string 또는 null | 네트워크 이름 |
| result.securityGroups | 선택 | array (object) | 보안 그룹 목록 |
| result.securityGroups[].securityGroupId | 선택 | string 또는 null | 보안 그룹 ID |
| result.securityGroups[].securityGroupName | 선택 | string 또는 null | 보안 그룹 이름 |
| result.attachedVolumes | 선택 | array (object) | 연결된 볼륨 목록 |
| result.attachedVolumes[].volumeId | 필수 | string | 볼륨 ID |
| result.attachedVolumes[].volumeName | 선택 | string 또는 null | 볼륨 이름 |
| result.advancedInfo | 선택 | object | 고급 정보 |
