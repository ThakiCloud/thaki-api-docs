...
중복 시 409 Conflict, 사용 가능하면 200 OK를 반환합니다.

## HTTP 요청

```http
GET https://<your-console-host>/api/v1/compute/images/name-availability
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| imageName | 필수 | string | 중복 검사할 이미지 이름 (1~255자, 인쇄 가능 문자, 선행/후행 공백 금지). 중복 검사할 이미지 이름 (1~255자, 인쇄 가능 문자, 선행/후행 공백 금지) |
