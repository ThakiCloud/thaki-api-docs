...
```http
PATCH https://<your-console-host>/api/v1/compute/storage/volume-backups/{volumeBackupId}
```
...
| 이름 | 위치 | 필수 | 형식 | 설명 |
|---|---|---|---|---|
| volumeBackupId | path | 필수 | string | 볼륨 백업 ID |
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| volumeBackupName | 선택 | string 또는 null | 볼륨 백업 이름 (1~255자, 인쇄 가능 문자, 선행/후행 공백 금지) |
| description | 선택 | string 또는 null | 볼륨 백업 설명 (null 전송 시 빈 값으로 설정, 미전송 시 기존 값 유지) |
