...
```http
PATCH https://<your-console-host>/api/v1/compute/instances/{instanceId}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| instanceName | 필수 | string | 인스턴스 이름 (1~255자, 인쇄 가능 문자, 선행/후행 공백 금지) |
| description | 선택 | string 또는 null | 설명 (free-text, 제어문자 및 &lt; &gt; 금지, 최대 255자; 미전송 시 기존 값 유지) |
