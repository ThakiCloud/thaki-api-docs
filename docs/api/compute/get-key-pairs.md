...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| sort | 선택 | string | 정렬 필드. 정렬 필드. 값: keyPairName, fingerprint, createdAt |
| order | 선택 | string | 정렬 방향. 정렬 방향. 값: asc, desc |
| keyPairName | 선택 | array (string) | KeyPair 이름 필터 (부분 일치, 다중 값 OR). KeyPair 이름 필터 (부분 일치, 다중 값 OR) |
| fingerprint | 선택 | array (string) | Fingerprint 필터 (부분 일치, 다중 값 OR). Fingerprint 필터 (부분 일치, 다중 값 OR) |
