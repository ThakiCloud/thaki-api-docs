...
기존 tags를 전체 교체합니다. 빈 리스트([])를 전달하면 모든 tags가 삭제됩니다.
OpenStack server tags 형식("key:value")으로 저장됩니다.

## HTTP 요청

```http
PUT https://<your-console-host>/api/v1/compute/instances/{instanceId}/tags
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| tags | 필수 | array (object) | 태그 목록 (최대 50개, Key 중복 불가, 빈 리스트로 전체 삭제) |
| tags[].key | 필수 | string | 태그 키 (1~256자, 영문 대/소문자, 숫자, 특수문자 가능). 길이 1~256 |
| tags[].value | 선택 | string 또는 null | 태그 값 (0~256자, null 또는 빈값 가능, 영문 대/소문자, 숫자, 특수문자 가능). 길이 0~256 |
