...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| sort | 선택 | string | 정렬 필드. 정렬 필드. 값: volumeBackupName, volumeBackupId, size, sourceVolumeName, backupMode, description, createdAt |
| order | 선택 | string | 정렬 방향. 정렬 방향. 값: asc, desc |
| volumeBackupName | 선택 | array (string) | 볼륨 백업 이름 필터 (부분 일치, 다중 값 OR). 볼륨 백업 이름 필터 (부분 일치, 다중 값 OR) |
| volumeBackupId | 선택 | array (string) | 볼륨 백업 ID 필터 (부분 일치, 다중 값 OR). 볼륨 백업 ID 필터 (부분 일치, 다중 값 OR) |
...
| backupMode | 선택 | array (string) | 백업 모드 필터 (전체 일치, 다중 값 OR). 백업 모드 필터 (전체 일치, 다중 값 OR) |
| size | 선택 | array (integer) | 볼륨 백업 크기 필터 GB 단위 (정확 일치, 다중 값 OR). 볼륨 백업 크기 필터 GB 단위 (정확 일치, 다중 값 OR) |
