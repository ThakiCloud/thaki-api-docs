...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| sort | 선택 | string | 정렬 필드. 값: volumeSnapshotName, volumeSnapshotId, size, volumeName, volumeType, description, createdAt |
| order | 선택 | string | 정렬 방향. 값: asc, desc |
| prioritizeAvailable | 선택 | boolean | available 상태 볼륨 스냅샷 우선 정렬. 기본값 false |
| volumeSnapshotName | 선택 | array (string) | 볼륨 스냅샷 이름 필터 (부분 일치, 다중 값 OR) |
| volumeSnapshotId | 선택 | array (string) | 볼륨 스냅샷 ID 필터 (부분 일치, 다중 값 OR) |
...
| size | 선택 | array (integer) | 볼륨 스냅샷 크기 필터 GB 단위 (정확 일치, 다중 값 OR) |
