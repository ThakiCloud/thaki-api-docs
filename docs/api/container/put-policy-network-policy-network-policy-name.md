...
YAML 스펙을 기반으로 Kubernetes 클러스터의 기존 NetworkPolicy를 수정합니다.

## HTTP 요청

```http
PUT https://<your-console-host>/api/v1/container/policy/network-policy/{network_policy_name}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| yamlContent | 필수 | string | NetworkPolicy 정의 YAML 문자열 |
...
| result.networkPolicyName | 필수 | string | 수정된 NetworkPolicy 이름 |
