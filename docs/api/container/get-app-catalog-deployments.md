...
| application | 선택 | string 또는 null | App Catalog application 필터. App Catalog application 필터 |
| clusterId | 선택 | integer 또는 null | 배포 대상 k8s_clusters.id 필터. 배포 대상 k8s_clusters.id 필터. 범위 1~ |
| namespace | 선택 | string 또는 null | 배포 네임스페이스 필터. 배포 네임스페이스 필터 |
| status | 선택 | string 또는 null | 배포 상태 필터. 배포 상태 필터 |
| keyword | 선택 | string 또는 null | application/userApp/clusterId/namespace 부분 일치 검색어. application/userApp/clusterId/namespace 부분 일치 검색어 |
| packageType | 선택 | string 또는 null | App Catalog 패키지 타입 필터. apps 또는 operators만 허용합니다.. App Catalog 패키지 타입 필터. apps 또는 operators만 허용합니다.. 값: apps, operators |
| sortBy | 선택 | string | 정렬 기준. 정렬 기준. 값: appName, chartName, lastDeployed, createdAt, updatedAt, application, userApp, status. 기본값 "updatedAt" |
| sortOrder | 선택 | string | 정렬 순서. 정렬 순서. 값: asc, desc. 기본값 "desc" |
