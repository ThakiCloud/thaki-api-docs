...
YAML 스펙을 기반으로 Kubernetes 클러스터의 기존 StorageClass를 수정합니다.

## HTTP 요청

```http
PUT https://<your-console-host>/api/v1/container/storage/storage-classes/{storage_class_name}
```
...
| 이름 | 위치 | 필수 | 형식 | 설명 |
|---|---|---|---|---|
| storage_class_name | path | 필수 | string | StorageClass 이름 |
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| yamlContent | 필수 | string | StorageClass 정의 YAML 문자열 |
...
| result.storageClassName | 필수 | string | 수정된 StorageClass 이름 |
