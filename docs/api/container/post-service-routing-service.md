...
```http
POST https://<your-console-host>/api/v1/container/service-routing/service
```
...
| result.serviceName | 필수 | string | 생성된 Service 이름 |
| result.namespace | 필수 | string | Service가 생성된 네임스페이스 |
| result.createdAt | 필수 | string (date-time) | 생성 시각 |
| result.message | 필수 | string | 추가 메시지 |
