...
```http
POST https://<your-console-host>/api/v1/container/workload/cronjobs/{cronjob_name}/resume
```
...
| result.cronjobName | 필수 | string | 재개된 CronJob 이름 |
| result.namespace | 필수 | string | CronJob이 속한 네임스페이스 |
| result.suspended | 필수 | boolean | 일시 중지 여부 (항상 False) |
