...
```http
PATCH https://<your-console-host>/api/v1/container/app-catalog/deployments/{deployment_id}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| config | 선택 | object 또는 null | tenant-values.yaml placeholder 치환 값. 제공하면 YAML을 재렌더링합니다 |
| yamlContent | 선택 | string 또는 null | 직접 수정한 values.yaml 내용 |
| status | 선택 | string 또는 null | 수정할 배포 상태 |
| statusMessage | 선택 | string 또는 null | 수정할 배포 상태 메시지 |
