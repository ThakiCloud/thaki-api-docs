...
PodDisruptionBudget 수정

YAML 스펙을 기반으로 Kubernetes 클러스터의 기존 PodDisruptionBudget를 수정합니다.

## HTTP 요청

```http
PUT https://<your-console-host>/api/v1/container/policy/pod-disruption-budget/{pdb_name}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| yamlContent | 필수 | string | PodDisruptionBudget 정의 YAML 문자열 |
...
| result.pdbName | 필수 | string | 수정된 PodDisruptionBudget 이름 |
