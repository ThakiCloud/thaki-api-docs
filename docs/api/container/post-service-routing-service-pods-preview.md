...
Service 생성 전에 selector로 매칭되는 Pod들을 조회합니다.
페이지네이션을 지원하며, page=0이면 전체 목록을 반환합니다.

## HTTP 요청

```http
POST https://<your-console-host>/api/v1/container/service-routing/service/pods/preview
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| selector | 필수 | object | Pod 라벨 셀렉터 (key-value 쌍) |
