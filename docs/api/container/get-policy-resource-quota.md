...
| filterName | 선택 | array (string) | 이름 필터 목록 (선택, 복수 지정 가능: filterName=quota&filterName=config). 부분일치(OR) 검색. 이름 필터 목록 (선택, 복수 지정 가능: filterName=quota&filterName=config). 부분일치(OR) 검색 |
