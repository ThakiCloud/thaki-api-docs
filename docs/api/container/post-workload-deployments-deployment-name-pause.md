...
```http
POST https://<your-console-host>/api/v1/container/workload/deployments/{deployment_name}/pause
```
...
| result.deploymentName | 필수 | string | 일시 중지된 Deployment 이름 |
| result.namespace | 필수 | string | Deployment가 속한 네임스페이스 |
| result.clusterId | 필수 | integer | 클러스터 ID |
| result.isPaused | 필수 | boolean | 일시 중지 상태 (True: 일시 중지됨) |
