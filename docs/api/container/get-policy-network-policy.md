...
```http
GET https://<your-console-host>/api/v1/container/policy/network-policy
```
...
| filterName | 선택 | array (string) | 이름 필터 목록 (선택, 복수 지정 가능: filterName=allow&filterName=deny). 부분일치(OR) 검색 |
| page | 선택 | integer | 페이지 번호 (1부터 시작, 0이면 전체 반환). 기본값 1. 범위 0~ |
...
| result.data[].name | 필수 | string | NetworkPolicy 이름 |
| result.data[].status | 선택 | string 또는 null | NetworkPolicy 상태 (조건 없음이면 null) |
| result.data[].namespace | 필수 | string | NetworkPolicy가 속한 네임스페이스 |
| result.data[].displayPodSelector | 필수 | string | Pod Selector 요약 문자열 (예: 'app=web (+2)') |
