...
```http
POST https://<your-console-host>/api/v1/container/workload/deployments/{deployment_name}/scale
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| replicas | 필수 | integer | 조정할 replica 수 (0 이상). 범위 0~ |
...
| result.deploymentName | 필수 | string | 스케일 조정된 Deployment 이름 |
| result.namespace | 필수 | string | Deployment가 속한 네임스페이스 |
| result.clusterId | 필수 | integer | 클러스터 ID |
| result.previousReplicas | 필수 | integer | 이전 replica 수 |
| result.currentReplicas | 필수 | integer | 현재 replica 수 |
