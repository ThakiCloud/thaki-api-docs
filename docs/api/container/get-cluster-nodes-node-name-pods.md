...
특정 Node에서 실행 중인 Pod 목록을 페이지네이션하여 조회합니다.
...
```http
GET https://<your-console-host>/api/v1/container/cluster/nodes/{node_name}/pods
```
...
| pageSize | 선택 | integer | 페이지 크기 (최소 1, 최대 100). 페이지 크기 (최소 1, 최대 100). 기본값 20. 범위 1~100 |
| filterName | 선택 | array (string) | Pod 이름 필터 목록 (선택, 복수 지정 가능, 부분일치 OR 검색, 대소문자 구분 없음). Pod 이름 필터 목록 (선택, 복수 지정 가능, 부분일치 OR 검색, 대소문자 구분 없음) |
