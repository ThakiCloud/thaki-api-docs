...
| filterName | 선택 | array (string) | Node 이름 필터 목록 (선택, 복수 지정 가능: filterName=wk&filterName=cp). 부분일치(OR) 검색. Node 이름 필터 목록 (선택, 복수 지정 가능: filterName=wk&filterName=cp). 부분일치(OR) 검색 |
| sortBy | 선택 | string | 정렬 기준 (name, version, os, createdAt). 정렬 기준 (name, version, os, createdAt). 값: name, version, os, createdAt. 기본값 "createdAt" |
