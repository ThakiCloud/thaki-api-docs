...
YAML 스펙을 기반으로 Kubernetes 클러스터의 기존 LimitRange를 수정합니다.

## HTTP 요청

```http
PUT https://<your-console-host>/api/v1/container/policy/limit-range/{limit_range_name}
```
...
| 이름 | 위치 | 필수 | 형식 | 설명 |
|---|---|---|---|---|
| limit_range_name | path | 필수 | string | LimitRange 이름. 길이 0~253 |
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| yamlContent | 필수 | string | LimitRange 정의 YAML 문자열 |
...
| result.limitRangeName | 필수 | string | 수정된 LimitRange 이름 |
