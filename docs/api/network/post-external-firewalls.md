...
```http
POST https://<your-console-host>/api/v1/network/external-firewalls
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| name | 필수 | string | 방화벽 이름 (표시용 메타데이터, AWX 미전달). 길이 1~ |
| externalNetworkId | 필수 | string | External Network UUID (= floating network). 길이 1~ |
| description | 선택 | string 또는 null | 방화벽 설명 (표시용 메타데이터, AWX 미전달) |
