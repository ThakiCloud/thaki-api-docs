...
```http
PUT https://<your-console-host>/api/v1/network/ports/{portId}
```
...
| fixedIps | 선택 | array (object) | 포트 고정 IP 목록 (빈 목록으로 설정 시 모든 고정 IP를 해제) |
| fixedIps[].subnetId | 필수 | string | 서브넷 ID |
| fixedIps[].ipAddress | 선택 | string 또는 null | 고정 IP 주소 |
| macAddress | 선택 | string 또는 null | MAC 주소 (입력하지 않으면 기존 값을 유지, null은 자동 재할당) |
...
| allowedAddressPairs | 선택 | array (object) | 허용 주소 쌍 목록 (빈 목록으로 설정 시 모든 항목을 제거) |
| allowedAddressPairs[].ipAddress | 필수 | string | 허용할 IP 주소 (CIDR 가능) |
| allowedAddressPairs[].macAddress | 선택 | string 또는 null | 허용할 MAC 주소 |
