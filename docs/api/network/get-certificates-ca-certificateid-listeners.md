...
```http
GET https://<your-console-host>/api/v1/network/certificates/ca/{certificateId}/listeners
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| status | 선택 | array (string) | Listener 상태 필터 (OctaviaStatus, 선택). Listener 상태 필터 (OctaviaStatus, 선택) |
| name | 선택 | array (string) | Listener 이름 필터. Listener 이름 필터 |
| id | 선택 | array (string) | Listener ID 필터. Listener ID 필터 |
| protocol | 선택 | array (string) | 프로토콜 필터. 프로토콜 필터 |
| port | 선택 | array (integer) | 포트 필터. 포트 필터 |
| loadBalancerName | 선택 | array (string) | LoadBalancer 이름 필터. LoadBalancer 이름 필터 |
| loadBalancerId | 선택 | array (string) | LoadBalancer ID 필터. LoadBalancer ID 필터 |
...
| sort | 선택 | string | 정렬 대상 컬럼. 정렬 대상 컬럼. 값: name, protocol, port, loadBalancerName |
