...
```http
POST https://<your-console-host>/api/v1/network/certificates/server
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| name | 필수 | string | 인증서 이름. 길이 1~255 |
| certificateContent | 필수 | string | 인증서 내용 (PEM 형식). 길이 1~ |
| privateKey | 필수 | string | 개인 키 (PEM 형식). 길이 1~ |
| certificateChain | 선택 | string 또는 null | 인증서 체인 (PEM 형식, optional) |
| domainName | 선택 | string 또는 null | 도메인 이름 |
| expiresAt | 선택 | string (date-time) | 만료 일시 |
