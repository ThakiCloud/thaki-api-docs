...
```http
PUT https://<your-console-host>/api/v1/network/routers/{routerId}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| name | 선택 | string 또는 null | 라우터 이름. 길이 1~255 |
| description | 선택 | string 또는 null | 라우터 설명 |
| externalGatewayInfo | 선택 | object 또는 null | 외부 게이트웨이 정보 (null이면 게이트웨이 제거) |
| externalGatewayInfo.networkId | 필수 | string | 외부 네트워크 ID. 길이 1~ |
| adminStateUp | 선택 | boolean 또는 null | 라우터 관리자 상태 |
