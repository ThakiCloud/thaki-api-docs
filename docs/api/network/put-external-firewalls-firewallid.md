# 방화벽 표시용 메타데이터(이름/설명) 수정

방화벽 표시용 메타데이터(이름/설명)를 수정합니다.

External network는 읽기 전용이며 AWX 프로비저닝과 무관한 메타데이터만 변경합니다.
description은 요청에 포함된 경우에만 반영합니다(미포함 시 기존 값 보존).

## HTTP 요청

```http
PUT https://<your-console-host>/api/v1/network/external-firewalls/{firewallId}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| name | 필수 | string | 변경할 방화벽 이름 (표시용 메타데이터, AWX 미전달). 길이 1~ |
| description | 선택 | string 또는 null | 변경할 방화벽 설명 (표시용 메타데이터, AWX 미전달) |
