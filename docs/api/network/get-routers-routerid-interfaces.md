# 라우터에 연결된 인터페이스 목록을 상세 조회

라우터에 연결된 인터페이스 목록을 상세 조회합니다.

## HTTP 요청

```http
GET https://<your-console-host>/api/v1/network/routers/{routerId}/interfaces
```
...
| sort | 선택 | string | 정렬 대상 컬럼. 값: networkName, networkId, name, createdAt |
| order | 선택 | string | 정렬 방향 asc/desc. 값: asc, desc |
| name | 선택 | array (string) | 서브넷 이름 필터 |
| id | 선택 | array (string) | 서브넷 ID 필터 (subnet_id) |
| subnetCidr | 선택 | array (string) | 서브넷 CIDR 필터 |
| networkName | 선택 | array (string) | 네트워크 이름 필터 |
| networkId | 선택 | array (string) | 네트워크 ID 필터 |
| allocationPools | 선택 | array (string) | 할당 풀 필터 (포함 일치) |
...
| result.data[].subnetId | 선택 | string 또는 null |  |
| result.data[].subnetName | 선택 | string 또는 null |  |
| result.data[].subnetCidr | 선택 | string 또는 null |  |
...
| result.data[].allocationPools | 선택 | array (string) |  |
