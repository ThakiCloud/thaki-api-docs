...
| sort | 선택 | string | 정렬 대상 컬럼. 정렬 대상 컬럼. 값: networkName, networkId, name, createdAt |
| order | 선택 | string | 정렬 방향 asc/desc. 정렬 방향 asc/desc. 값: asc, desc |
| name | 선택 | array (string) | 서브넷 이름 필터. 서브넷 이름 필터 |
| id | 선택 | array (string) | 서브넷 ID 필터 (subnet_id). 서브넷 ID 필터 (subnet_id) |
...
| allocationPools | 선택 | array (string) | 할당 풀 필터 (포함 일치). 할당 풀 필터 (포함 일치) |
