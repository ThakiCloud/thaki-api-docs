...
```http
POST https://<your-console-host>/api/v1/network/security-groups/{securityGroupId}/rules
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| direction | 필수 | string | 트래픽 방향 (INGRESS/EGRESS). 값: INGRESS, EGRESS |
| etherType | 선택 | string | 이더넷 타입 (IPv4/IPv6). 기본값: IPv4. 값: IPv4, IPv6 |
| protocol | 필수 | string | 프로토콜 (TCP/UDP/ICMP 등). 값: TCP, UDP, ICMP, ANY |
| portRangeMin | 선택 | integer 또는 null | 포트 범위 시작 (tcp/udp). 범위 1~65535 |
| portRangeMax | 선택 | integer 또는 null | 포트 범위 끝 (tcp/udp). 범위 1~65535 |
| icmpType | 선택 | integer 또는 null | ICMP Type. 범위 0~255 |
| icmpCode | 선택 | integer 또는 null | ICMP Code. 범위 0~255 |
| remoteIpPrefix | 선택 | string 또는 null | 원격 IP 프리픽스 (CIDR) |
| remoteGroupId | 선택 | string 또는 null | 원격 Security Group ID |
| description | 선택 | string 또는 null | 설명. 길이 0~255 |
