...
```http
GET https://<your-console-host>/api/v1/network/ports/connectable
```
...
| sort | 선택 | string | 정렬 대상 컬럼 (기본값: connectable). 정렬 대상 컬럼 (기본값: connectable). 값: createdAt, name, id, networkName, connectable |
