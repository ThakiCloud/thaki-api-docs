...
```http
PUT https://<your-console-host>/api/v1/network/floating-ips/{floatingIpId}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| portId | 선택 | string 또는 null | 연결할 포트 ID (null이면 연결 해제, 생략 시 변경 없음) |
| fixedIpAddress | 선택 | string 또는 null | 포트의 고정 IP 주소 |
| instanceId | 선택 | string 또는 null | Instance 연결 QoS 자동 적용 대상 Nova instance ID |
| description | 선택 | string 또는 null | Floating IP 설명 (생략 시 변경 없음) |
