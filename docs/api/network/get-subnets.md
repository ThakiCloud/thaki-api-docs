...
| id | 선택 | array (string) | 서브넷 ID 필터 |
| cidr | 선택 | array (string) | 서브넷 CIDR 필터 |
| gatewayIp | 선택 | array (string) | 게이트웨이 IP 필터 |
| portCount | 선택 | integer 또는 null | 포트 수 필터. 범위 0~ |
...
| sort | 선택 | string | 정렬 대상 컬럼. 값: name, gatewayIp, portCount, createdAt |
| order | 선택 | string | 정렬 방향 asc/desc. 값: asc, desc |
| networkId | 선택 | string 또는 null | 필터링할 네트워크 ID |
