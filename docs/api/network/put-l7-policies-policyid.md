...
```http
PUT https://<your-console-host>/api/v1/network/l7-policies/{policyId}
```
...
| behavior | 선택 | string 또는 null | L7 Policy 동작(behavior). 값: REDIRECT_TO_POOL, REDIRECT_TO_URL, REDIRECT_PREFIX, REJECT |
| position | 선택 | integer 또는 null | L7 Policy 우선순위. 범위 1~ |
...
| redirectPrefix | 선택 | string 또는 null | 리다이렉트 URL prefix (REDIRECT_PREFIX 시 필수) |
| redirectHttpCode | 선택 | integer 또는 null | 리다이렉트 HTTP 코드. 범위 300~399 |
