...
```http
PUT https://<your-console-host>/api/v1/network/health-monitors/{healthMonitorId}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| delay | 선택 | integer 또는 null | Health check 간격 (초). 범위 1~ |
| timeout | 선택 | integer 또는 null | Health check 타임아웃 (초). 범위 1~ |
| maxRetries | 선택 | integer 또는 null | 최대 재시도 횟수. 범위 1~ |
