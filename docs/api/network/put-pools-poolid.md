...
```http
PUT https://<your-console-host>/api/v1/network/pools/{poolId}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| tlsEnabled | 선택 | boolean 또는 null | Backend TLS 활성화 여부 |
| name | 선택 | string 또는 null | Pool 이름 |
| description | 선택 | string 또는 null | Pool 설명 |
| adminStateUp | 선택 | boolean 또는 null | Pool 관리자 상태 |
| lbAlgorithm | 선택 | string 또는 null | 로드밸런싱 알고리즘 (L4 Listener인 경우 SOURCE_IP_PORT만 지원). 값: ROUND_ROBIN, LEAST_CONNECTIONS, SOURCE_IP, SOURCE_IP_PORT |
