...
| sort | 선택 | string | 정렬 대상 컬럼. 정렬 대상 컬럼. 값: id, firewallId, status, createdAt, updatedAt, syncedAt |
| order | 선택 | string | 정렬 방향 asc/desc. 정렬 방향 asc/desc. 값: asc, desc |
| id | 선택 | array (string) | 규칙 ID 필터. 규칙 ID 필터 |
| firewallId | 선택 | array (string) | 방화벽 ID 필터. 방화벽 ID 필터 |
| srcCidr | 선택 | array (string) | Source CIDR 필터. Source CIDR 필터 |
| dstCidr | 선택 | array (string) | Dest CIDR 필터. Dest CIDR 필터 |
| protocol | 선택 | array (string) | protocol 필터. protocol 필터 |
