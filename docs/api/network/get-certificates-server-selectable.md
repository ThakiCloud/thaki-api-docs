...
```http
GET https://<your-console-host>/api/v1/network/certificates/server/selectable
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| status | 선택 | array (string) | 인증서 만료 상태 필터 (VALID, EXPIRING_SOON, EXPIRED) |
| name | 선택 | array (string) | 인증서 이름 필터 |
| id | 선택 | array (string) | 인증서 ID 필터 |
| listener | 선택 | array (string) | 연결된 리스너 ID 또는 이름 필터 |
...
| createdAtGte | 선택 | string 또는 null | 생성 시작 일시 (포함), ISO8601 YYYY-MM-DD |
| createdAtLte | 선택 | string 또는 null | 생성 종료 일시 (포함), ISO8601 YYYY-MM-DD |
...
| sort | 선택 | string | 정렬 대상 컬럼 (기본값: expiresAt). 값: expiresAt, createdAt, id, name |
| order | 선택 | string | 정렬 방향 asc/desc (기본값: asc). 값: asc, desc |
| san | 선택 | array (string) | SAN 필터 |
...
| result.data[].privateKeyRef | 선택 | string 또는 null |  |
| result.data[].certificateChainRef | 선택 | string 또는 null |  |
