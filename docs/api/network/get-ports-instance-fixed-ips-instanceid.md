...
```http
GET https://<your-console-host>/api/v1/network/ports/instance-fixed-ips/{instanceId}
```
...
| sort | 선택 | string 또는 null | 정렬 대상 컬럼 (미지정 시 connectable 우선). 정렬 대상 컬럼 (미지정 시 connectable 우선). 값: fixedIp, status, networkName |
| order | 선택 | string | 정렬 방향 asc/desc. 정렬 방향 asc/desc. 값: asc, desc |
| floatingIpId | 선택 | string 또는 null | 검증할 Floating IP ID (선택). 검증할 Floating IP ID (선택) |
...
| result.data[].portId | 필수 | string 또는 null |  |
