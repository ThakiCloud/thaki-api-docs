...
| sort | 선택 | string | 정렬 대상 컬럼. 값: id, name, externalNetworkId, externalNetworkName, status, createdAt, updatedAt, syncedAt |
| order | 선택 | string | 정렬 방향 asc/desc. 값: asc, desc |
| id | 선택 | array (string) | 방화벽 ID 필터 |
| name | 선택 | array (string) | 방화벽 이름 필터(부분일치) |
| externalNetworkId | 선택 | array (string) | External network ID 필터 |
| externalNetworkName | 선택 | array (string) | External network 이름 필터(부분일치) |
