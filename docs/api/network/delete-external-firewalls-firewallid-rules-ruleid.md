...
허용 규칙 1개 삭제 요청을 접수합니다.

규칙 id(ruleId)는 전역 유일하므로 삭제는 ruleId만으로 수행한다. 경로의
firewallId는 리소스 계층 표현용이다.

## HTTP 요청

```http
DELETE https://<your-console-host>/api/v1/network/external-firewalls/{firewallId}/rules/{ruleId}
```
