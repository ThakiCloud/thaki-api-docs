...
domain/resource 기반 테이블 컬럼 설정을 생성/수정(Upsert)합니다.
...
```http
PUT https://<your-console-host>/api/v1/network/columns-config/{domain}/{resource}
```
...
| 이름 | 필수 | 형식 | 설명 |
|---|---|---|---|
| pageSize | 선택 | integer 또는 null | 페이지당 표시할 항목 수 (10/20/50/100). 값: 10, 20, 50, 100 |
| columns | 선택 | array (object) | 컬럼 설정 배열(배열 순서가 order) |
| columns[].key | 필수 | string | 컬럼 key (예: created_at) |
| columns[].visible | 필수 | boolean | 표시 여부 |
| columns[].order | 필수 | integer | 정렬 순서(0..n-1). 범위 0~ |
